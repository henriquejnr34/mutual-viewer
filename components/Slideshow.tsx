...
const Slideshow: React.FC<SlideshowProps> = ({ mutuals, onReset }) => {
  ...
  const handleControlClick = (direction: 'next' | 'prev') => {
    ...
    setTimeout(() => {
      if (direction === 'next') {
        setCurrentIndex((prevIndex) => Math.min(prevIndex + 1, mutuals.length - 1));
      } else {
  ...
  const isAtEnd = currentIndex === mutuals.length - 1;
  const canShowNext = !isAtEnd;
